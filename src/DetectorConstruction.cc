...
DetectorConstruction::DetectorConstruction(G4bool enableOpticalModel)
    : fMessenger(std::make_unique<G4GenericMessenger>(
      ...
      fEnableOpticalModel(enableOpticalModel) {
...
G4double DetectorConstruction::GetCrystalCenterZ() const {
  return fCrystalPhysical == nullptr
             ? 0.0
             : fCrystalPhysical->GetObjectTranslation().z();
}
...
G4VPhysicalVolume* DetectorConstruction::Construct() {
  ...
  if (fEnableOpticalModel &&
      vacuum->GetMaterialPropertiesTable() == nullptr) {
  ...
  if (gagg == nullptr) {
    ...
    if (fEnableOpticalModel) {
      ...
      gaggMpt->AddProperty("SCINTILLATIONCOMPONENT1",
                           scintillationEnergies,
  ...
  if (fEnableOpticalModel &&
      reflector->GetMaterialPropertiesTable() == nullptr) {
  ...
  if (fEnableOpticalModel && air->GetMaterialPropertiesTable() == nullptr) {
  ...
  if (fEnableOpticalModel &&
      experimentBlack->GetMaterialPropertiesTable() == nullptr) {
  ...
  if (fEnableOpticalModel &&
      experimentPmtWindow->GetMaterialPropertiesTable() == nullptr) {
  ...
  if (fGeometryMode == "paper") {
    ...
    const auto upperShoulderZ =
        crystalHalfZ - upperShoulderHalfZ;
    const auto lowerShoulderZ =
        -crystalHalfZ + lowerShoulderHalfZ;
  ...
  G4cout << "[geometry] mode=" << fGeometryMode
         << " optical_model=" << (fEnableOpticalModel ? "on" : "off")
...
void DetectorConstruction::ValidateStageBSurfaces() {
  ...
  const auto esrPass =
      ...
      esr->GetType() == dielectric_metal &&
      esr->GetFinish() == polished;
